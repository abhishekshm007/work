...
@WebServlet("/GetPurchasedTestQuestion")
public class GetPurchasedTestQuestion extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
   
    public GetPurchasedTestQuestion() {
        super();
       
    }
    private static final Logger LOGGER = LoggerFactory.getLogger(GetPurchasedTestQuestion.class);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		HttpSession session=request.getSession();
			...
			psList.add(psDTO);
				...
				StringBuilder sql2 = new StringBuilder(SQLConstants.COUNT_MY_PURCHASED_QUESTION.replace("instituteid",((SessionDTO)session.getAttribute("user")).getCurrentInstituteId().toString()));
				...
				if(totalRecords>0){
				StringBuilder sql1 = new StringBuilder(SQLConstants.GET_MY_PURCHASED_TEST_QUESTIONS);
				if(request.getParameter("sSearch") != null) {
					String search=request.getParameter("sSearch");
