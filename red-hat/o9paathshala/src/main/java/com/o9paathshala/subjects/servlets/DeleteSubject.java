...
public class DeleteSubject extends HttpServlet {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(DeleteSubject.class);
		/**
	...
	HttpSession session=request.getSession();
		...
		try {
			data=dao.cud(SQLConstants.DELETE_SUBJECT.replace("instituteid",((SessionDTO)session.getAttribute("user")).getCurrentInstituteId().toString()), psList);
			if(data>0){
				result=true;
			}
