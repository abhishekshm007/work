...
public class DeleteFaculty extends HttpServlet {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(DeleteFaculty.class);
	/**
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		psDTO.setPosition(1);
		psDTO.setValue(Integer.parseInt(request.getParameter("facultyid")));
		psList.add(psDTO);
			data=dao.cud(SQLConstants.DELETE_FACULTY.replace("instituteid",((SessionDTO)session.getAttribute("user")).getCurrentInstituteId().toString()), psList);
			if(data>0){
				result=true;
			}
