...
public class NewTest extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			psList.add(psDTO);
			cq.setQuery(SQLConstants.CREATE_TEST.replace("instituteid",((SessionDTO)session.getAttribute("user")).getCurrentInstituteId().toString()));
			cq.setPsList(psList);
			cqList.add(cq);
			...
			for(String section:sections){
				if(section.length()>0){
					...
					psDTO.setDataType(DBConstants.STRING);
					psDTO.setPosition(1);
					psDTO.setValue(section);
					psList.add(psDTO);
					cq=new ConfiguredQuery();
					cq.setPsList(psList);
					cq.setQuery(SQLConstants.CREATE_SECTION.replace("instituteid",((SessionDTO)session.getAttribute("user")).getCurrentInstituteId().toString()));
					cqList.add(cq);
					cq=new ConfiguredQuery();
					cq.setPsList(null);
					cq.setQuery(SQLConstants.TEST_SECTION.replace("instituteid",((SessionDTO)session.getAttribute("user")).getCurrentInstituteId().toString()));
					cqList.add(cq);
				}
			}
